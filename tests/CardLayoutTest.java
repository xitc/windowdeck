...
public final class CardLayoutTest {
 public static void main(String[] args){
  ...
  for(int[] viewport:viewports)for(int mode=0;mode<2;mode++)for(int n=1;n<=3;n++)for(int p=0;p<n;p++)for(int mask=0;mask<(1<<n);mask++){
   int w=viewport[0],h=viewport[1],d=w>1000?4:1,gap=6*d,badge=22*d;
   int sideH=PaneLayout.previewHeight(w,h,gap,84*d,badge,mode);
   int[][] baseline=PaneLayout.compute(w,h,n,p,gap,84*d,sideH,mode);
   int[][] sizes=new int[n][2];
   for(int i=0;i<n;i++)sizes[i]=(mask&(1<<i))==0?new int[]{1440,3168}:new int[]{3168,1440};
   CardLayout.Result result=CardLayout.compute(w,h,p,gap,84*d,sideH,mode,badge,48*d,sizes);
   for(int i=0;i<n;i++){
   ...
   if(n<3){inside(result.add,w,h);for(int[] card:result.cards)separate(result.add,card);check(result.add[3]<=result.add[2],"compact add");}
  ...
  int sh=PaneLayout.previewHeight(360,792,6,84,22,0);
  CardLayout.Result landscape=CardLayout.compute(360,792,0,6,84,sh,0,22,48,new int[][]{{3168,1440},{3168,1440}});
  check(landscape.cards[0][3]<130,"landscape main no tall empty frame");
  check(landscape.cards[1][3]==60,"landscape preview follows its own ratio plus badge");
  check(landscape.add[1]==72,"add follows compact preview");
  CardLayout.Result pending=CardLayout.compute(360,792,0,6,84,sh,0,22,48,new int[][]{{0,0},{0,0}});
  check(pending.cards[0][3]==780,"uninitialized render keeps allocation");
  for(int mode=0;mode<2;mode++){
   int sideW=mode==0?56:84;
   int height=PaneLayout.previewHeight(360,792,6,sideW,22,mode);
   int[] rotated=OrientationPolicy.presentationSize(3168,1440,true);
   CardLayout.Result reference=CardLayout.compute(360,792,0,6,sideW,height,mode,22,48,new int[][]{rotated,{1440,3168}});
   ...
   if(mode==0)check(reference.cards[1][0]+reference.cards[1][2]<reference.cards[0][0],"reference left rail with main on right");
   else check(reference.cards[1][1]+reference.cards[1][3]<reference.cards[0][1],"reference top rail with main below");
   // Keyboard compression changes fit, never the decision to rotate.
   CardLayout.Result ime=CardLayout.compute(360,220,0,6,sideW,height,mode,22,48,new int[][]{rotated,{1440,3168}});
